...
namespace Hooks
{
	...
	void Initialise()
	{
		...
		m_pSurface->SetFontGlyphSet(draw.fonts.esp, XorStr("Verdana"), 12, 600, 0, 0, FONTFLAG_OUTLINE);

		draw.fonts.esp_extra = m_pSurface->FontCreate();
		m_pSurface->SetFontGlyphSet(draw.fonts.esp_extra, XorStr("Smallest Pixel-7"), 10, 400, 0, 0, (int)FONTFLAG_OUTLINE);

		draw.fonts.esp_small = m_pSurface->FontCreate();
		m_pSurface->SetFontGlyphSet(draw.fonts.esp_small, XorStr("Smallest Pixel-7"), 11, 230, 0, 0, (int)FONTFLAG_OUTLINE);

		draw.fonts.esp_drops = m_pSurface->FontCreate();
		m_pSurface->SetFontGlyphSet(draw.fonts.esp_drops, XorStr("Smallest Pixel-7"), 14, 400, 0, 0, (int)FONTFLAG_ANTIALIAS | (int)FONTFLAG_OUTLINE);

		draw.fonts.esp_icons = m_pSurface->FontCreate();
		m_pSurface->SetFontGlyphSet(draw.fonts.esp_icons, XorStr("undefeated"), 16, 600, 0, 0, (int)FONTFLAG_OUTLINE);

		draw.fonts.indicator = m_pSurface->FontCreate();
		m_pSurface->SetFontGlyphSet(draw.fonts.indicator, XorStr("Verdana"), 24, 700, 0, 0, (int)FONTFLAG_ANTIALIAS | (int)FONTFLAG_OUTLINE);
